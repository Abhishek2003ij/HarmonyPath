...
if 'bed_df' not in st.session_state:
    st.session_state.bed_df = pd.DataFrame(columns=["Bed Number", "Patient Name", "Disease", "Status"])
...
if selected == 'Admin Login':
    ...
    st.write("This section is reserved for Admin users to manage records and view reports.")
...
# Check for admin login and reset if patient login is selected
if 'admin_logged_in' in st.session_state and st.session_state.admin_logged_in:
    # Bed Matrix Operations page
    st.title("Bed Matrix Operations")
    
    # Display current bed allocations
    st.subheader("Current Bed Allocations")
    st.dataframe(bed_df)

    # Section to add a new bed allocation
    st.subheader("Add New Bed Allocation")
    with st.form(key='bed_allocation_form'):
        bed_number = st.number_input("Bed Number", min_value=1, value=1)
        patient_name = st.text_input("Patient Name")
        disease = st.selectbox("Disease", ["Diabetes", "Heart Disease", "Parkinson", "Liver", "Hepatitis", "Lung Cancer", "Chronic Kidney", "Breast Cancer"])
        status = st.selectbox("Status", ["Occupied", "Available"])
        
        submit_button = st.form_submit_button("Add Allocation")
        ...
        if submit_button:
            ...
            st.session_state.bed_df = pd.concat([bed_df, new_row], ignore_index=True)  # Update the DataFrame in session state
            st.success("New bed allocation added successfully!")

    # Optionally, add a button to log out
    if st.button("Logout"):
        st.session_state.admin_logged_in = False
        st.experimental_rerun()  # Rerun to go back
